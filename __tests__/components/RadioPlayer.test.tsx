...
jest.mock('@/app/components/layout/RadioLayout', () => ({
  RadioLayout: ({
    ...
    <div data-testid="radio-layout">
      ...
      <button data-testid="previous" onClick={onPrevious}>
        Previous
      </button>
      <button data-testid="select-track" onClick={() => onTrackSelect('track2')}>
        Select Track
      </button>
    </div>
  ),
...
describe('RadioPlayer', () => {
  ...
  it('handles track end events for auto-advance', async () => {
    ...
    // Should call validateTracks for next track
    await waitFor(
      () => {
        expect(mockValidateTracks).toHaveBeenCalled();
      },
      { timeout: 3000 }
    );
  });
